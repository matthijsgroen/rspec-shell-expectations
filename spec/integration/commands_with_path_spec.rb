...
describe 'scenario where command with path is mocked' do
  ...
  context 'same command, different path' do
    let(:script) do
      <<-SCRIPT
      command
      /absolute/path/to/command
    ...
    let(:stubbed_env) { create_stubbed_env }
    let!(:regular_command_mock) { stubbed_env.stub_command('command') }
    let!(:absolute_path_mock) { stubbed_env.stub_command('/absolute/path/to/command') }
    let!(:relative_path_mock) { stubbed_env.stub_command('relative/path/to/command') }

    it 'regular command is mocked' do
      stubbed_env.execute(script_path)

      expect(regular_command_mock).to be_called
    end

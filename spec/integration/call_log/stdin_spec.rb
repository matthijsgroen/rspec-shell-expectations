  ...
  context '#stdin' do
    context 'when given multiple command calls' do
      ...
      it 'matches for exact matches' do
        expect(first_command.with_args('first_argument', 'second_argument').stdin).to eql 'first_call'
      end

      it 'matches for anything matches' do
        expect(first_command.with_args(anything, anything, 'third_argument').stdin).to eql 'second_call'
      end

      it 'matches for any_args matches' do
        expect(first_command.with_args(any_args).stdin).to eql 'first_call'
      end

      it 'matches for other types of RSpec::Mock::ArgumentMatcher matches' do
        expect(first_command.with_args(instance_of(String), instance_of(String)).stdin).to eql 'first_call'
      end

      it 'matches for regex matches' do
        expect(first_command.with_args(/f..st_argument/, /se..nd_argument/, /.*/).stdin).to eql 'second_call'
      end
    end
  end
end
